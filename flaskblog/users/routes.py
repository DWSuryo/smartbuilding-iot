...
def register():
    ...
    if form.validate_on_submit():
        ...
        db.session.add(user)
        db.session.commit() # account has been created. # Then flash message (appears one time each execution): string message then css style)
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user and bcrypt.check_password_hash(user.password, form.password.data):
            ...
            next_page = request.args.get('next') # proceeds to next page
...
def account():
    ...
    if form.validate_on_submit():
        ...
        if form.picture.data:
            ...
            if os.path.exists(prev_picture) and os.path.basename(prev_picture) != 'default_pp.png':
